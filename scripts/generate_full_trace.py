...
def instrumented_apply_ascii_noising(text, sigma):
    ...
    for c in text:
        ...
        if is_printable:
            ...
            log(f'applyAsciiNoising char: "{c}", char_code: {char_code}, is_printable: {is_printable}, rand_float_1: {rand_float:.17f}, should_noise: {should_noise}')
            if should_noise:
...
if __name__ == "__main__":
    ...
    # Get the initial state from get_all_seed_states.py
    command = ['python', 'scripts/get_all_seed_states.py', str(seed)]
    result = subprocess.run(command, capture_output=True, text=True)
    states = json.loads(result.stdout)

    # Decode and set the state for the standard 'random' library
    std_state_b64 = states['std_random']['state_b64']
    std_state_bytes = base64.b64decode(std_state_b64)
    std_state_array = np.frombuffer(std_state_bytes, dtype=np.uint32)
    std_state_index = states['std_random']['index']

    # Correctly reconstruct the state tuple for random.setstate
    state_tuple = tuple(std_state_array.tolist()) + (std_state_index,)
    random.setstate((3, state_tuple, None))


    # Decode and set the state for 'numpy.random'
    np_state_b64 = states['np_random']['state_b64']
    np_state_bytes = base64.b64decode(np_state_b64)
    np_state_array = np.frombuffer(np_state_bytes, dtype=np.uint32)
    np_state_index = states['np_random']['index']

    # Correctly reconstruct the state tuple for np.random.set_state
    np.random.set_state(('MT19937', np_state_array, np_state_index, 0, 0.0))
