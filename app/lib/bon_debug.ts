...
const fmt = (n: number) => n.toFixed(17);

// Helper to match Python's json.dumps default formatting (spaces after separators)
function pyJson(obj: any): string {
    return JSON.stringify(obj).replace(/,/g, ', ');
}

function getPythonState(rng: PythonRandomProvider): any[] {
    const [mt, index] = rng.getState();
    // Python's random.getstate() returns (3, (tuple(mt), index), None)
    return [3, [mt, index], null];
}
...
export function applyWordScrambling(text: string, sigma: number, rng: PythonRandomProvider, logger?: Logger): string {
    ...
    logger?.(`applyWordScrambling words: ${pyJson(words)}`);
    ...
    for (const word of words) {
        if (word.length > 3) {
            ...
            // Python prints True/False for bools
            logger?.(`applyWordScrambling word: "${word}", rand_float: ${fmt(rand_float)}, should_scramble: ${should_scramble ? 'True' : 'False'}`);

            if (should_scramble) {
                const chars = Array.from(word);
                const middle_chars = chars.slice(1, -1);
                logger?.(`applyWordScrambling middle_chars before shuffle: ${pyJson(middle_chars)}`);
                logger?.(`applyWordScrambling PRE_SHUFFLE_STATE: ${pyJson(getPythonState(rng))}`);

                rng.std_shuffle(middle_chars);

                logger?.(`applyWordScrambling POST_SHUFFLE_STATE: ${pyJson(getPythonState(rng))}`);
                logger?.(`applyWordScrambling middle_chars after shuffle: ${pyJson(middle_chars)}`);
...
export function applyRandomCapitalization(text: string, sigma: number, rng: PythonRandomProvider, logger?: Logger): string {
    ...
    for (const c of text) {
        if (/[a-zA-Z]/.test(c)) {
            ...
            logger?.(`applyRandomCapitalization char: "${c}", rand_float: ${fmt(rand_float)}, should_capitalize: ${should_capitalize ? 'True' : 'False'}`);
...
export function applyAsciiNoising(text: string, sigma: number, rng: PythonRandomProvider, logger?: Logger): string {
    ...
    for (const c of text) {
        ...
        if (isPrintable(char_code)) {
            ...
            logger?.(`applyAsciiNoising char: "${c}", char_code: ${char_code}, is_printable: True, rand_float_1: ${fmt(rand_float_1)}, should_noise: ${should_noise ? 'True' : 'False'}`);

            if (should_noise) {
                const rand_float_2 = rng.std_random();
                const perturbation = rand_float_2 < 0.5 ? -1 : 1;
                const new_char_code = char_code + perturbation;
                logger?.(`applyAsciiNoising rand_float_2: ${fmt(rand_float_2)}, perturbation: ${perturbation}, new_char_code: ${new_char_code}`);
