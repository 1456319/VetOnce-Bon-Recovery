    ...
    for (const word of words) {
        if (word.length > 3) {
            ...
            if (should_scramble) {
                ...
                logger?.(`applyWordScrambling PRE_SHUFFLE_STATE: ${JSON.stringify(getPythonState(rng))}`);

                rng.std_shuffle(middle_chars);

                logger?.(`applyWordScrambling POST_SHUFFLE_STATE: ${JSON.stringify(getPythonState(rng))}`);
